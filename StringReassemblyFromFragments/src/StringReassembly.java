...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        assert 0 <= overlap && overlap <= str1.length()
                ...
                && str1.regionMatches(str1.length() - overlap, str2, 0,
                        overlap) : "Violation of: OVERLAPS(str1, str2, overlap)";
        // Concatenates 'str1' (excluding its overlapping part) with 'str2'.
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Iterate over the set to find substrings
        for (String existingStr : strSet) {
            if (str.contains(existingStr)) {
                // If str contains an existing string, mark for removal
                toRemove.add(existingStr);
            } else if (existingStr.contains(str)) {
                // If str is a substring of an existing string, set flag
                isSubstring = true;
            }
        }

        // Add str to the set if it's not a substring of any existing string
        if (!isSubstring) {
            strSet.add(str);
            // Remove strings that are substrings of str
            for (String remStr : toRemove) {
                strSet.remove(remStr);
            }
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Reads each line from the input and adds it to the set, avoiding any substrings.
        while (!input.atEOS()) {
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int n = 0; n < text.length(); n++) {
            // If the current character is '~', print a newline.
            if (text.charAt(n) == '~') {
                out.println();
            } else {
