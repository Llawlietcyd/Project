...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!m.isZero()) {
            ...
            reduceToGCD(n, m); // Recursive call
        }
        // If m is zero, do nothing; n already contains the GCD
    ...
    public static boolean isEven(NaturalNumber n) {
         ...
         */
        boolean even = false;
        // Copy n to avoid modifying it
        NaturalNumber nCopy = new NaturalNumber2(n);
        // Number 2 as a NaturalNumber
        NaturalNumber two = new NaturalNumber2(2);
        // If division by 2 leaves no remainder, it's even
        if (nCopy.divide(two).isZero()) {
            even = true;
        }
        return even; // Return evenness result
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {
            ...
            // Temporary holders for the original values of n and p
            NaturalNumber base = new NaturalNumber2(n);
            NaturalNumber exp = new NaturalNumber2(p);

            // Recursive call with p halved to use the fast exponentiation method
            exp.divide(new NaturalNumber2(2));
            powerMod(n, exp, m); // Recursive call: n = n^(p/2) mod m

            // Squaring the result of the recursive call
            n.power(2);
            n.copyFrom(n.divide(m)); // Ensuring the result is modulo m

            // If the original exponent was odd, multiply once more by the original base
            if (!isEven(exp)) {
                n.multiply(base); // n = n * original base
                n.copyFrom(n.divide(m)); // n = n mod m
            }
        }
    }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber wSquared = w.newInstance();
        wSquared.copyFrom(w);
        wSquared.power(2);
        wSquared.divide(n);
        boolean condition1 = wSquared.compareTo(new NaturalNumber2(1)) == 0;
        ...
        NaturalNumber wToNMinusOneModN = w.newInstance();
        wToNMinusOneModN.copyFrom(w);
        powerMod(wToNMinusOneModN, nMinusOne, n);
        boolean condition2 = wToNMinusOneModN.compareTo(one) != 0;

        // The witness to compositeness is true if either condition1 or condition2 is true
        boolean isWitness = condition1 || condition2;
        return isWitness;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        final int numWitnesses = 50; // Number of witness candidates to check
        ...
        if (n.compareTo(two) == 0 || n.compareTo(three) == 0) {
            ...
            isPrime = false; // Evens greater than 2 are not prime
        } else {
            NaturalNumber range = new NaturalNumber2(n);
            range.subtract(two); // Adjust range to n-2
            int i = 0; // Initialize counter for while loop
            // While not proven composite and still witnesses to check
            while (isPrime && i < numWitnesses) {
                // Generate a random number in the range [2, n-2]
                NaturalNumber randomNum = randomNumber(range);
                randomNum.add(two); // Shift the range to [2, n-1]
                if (randomNum.compareTo(n) < 0
                        && isWitnessToCompositeness(randomNum, n)) {
                    isPrime = false; // Found a witness, n is composite
                }
                i++; // Increment the counter
            }
        }
        return isPrime; // Return the primality result
    }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        // When the loop exits, n is a likely prime
    }
